...
// Zero-credit capture: the calling agent supplies the classification itself
// (kind, summary, tags), so no InvokeLLM runs server-side. Used by the MCP
// `remember` tool where the client model is perfectly capable of curating.
export async function captureDirect(cfg, { content, summary, kind, tags, repo, agent }) {
  const res = await makeClient(cfg).functions.invoke("seed-direct", {
    device_key: cfg.device_key,
    repo,
    agent: agent || "claude-code",
    source: "mcp",
    memories: [{ content, summary, kind, tags: tags || [], confidence: 0.75 }],
  });
  return res.data;
}

// Deliberately retire an outdated rule (action "forget") or replace it with a
// new pre-classified version (action "revise"). Zero credits either way.
export async function revise(cfg, { repo, targetId, targetQuery, action, replacement, agent }) {
  const res = await makeClient(cfg).functions.invoke("revise", {
    device_key: cfg.device_key,
    repo,
    target_id: targetId,
...
export async function check(cfg, { action, repo }) {
  const res = await makeClient(cfg).functions.invoke("check", {
    action,
    repo,
    device_key: cfg.device_key,
  });
...
export async function distill(cfg, { text, repo, agent }) {
  const res = await makeClient(cfg).functions.invoke("distill", {
    text,
    repo,
    agent,
    device_key: cfg.device_key,
