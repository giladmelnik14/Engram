...
| **CLI** | `engram learn "…"`, `engram recall "…"`, `engram watch` — a plain Node CLI, no browser, straight to the backend. |
| **MCP server** | `recall` + `remember` tools so Claude Code, Cursor, and any MCP client plug into the same memory. |
